...
export const HideSingleOperator: Story = () => {
  ...
  return (
    <div className="p-4">
      ...
      <p className="text-sm text-gray-600 mb-4">
        When hideSingleOperator is true, the operator is hidden if only one operator is available.
        Compare: Priority (1 operator, hidden) vs Status (2 operators, shown).
      </p>
      <TokenizedSearchInput
        fields={TOKEN_DISPLAY_FIELDS.auto.map((f) => ({ ...f, hideSingleOperator: true }))}
        defaultValue="priority:is:high status:is:active"
        onSubmit={setResult}
        placeholder="Compare operator display..."
      />
      <ResultDisplay result={result} />
      <div className="mt-3 p-3 bg-yellow-50 rounded text-xs text-yellow-800">
        <strong>Compare:</strong> "Priority: high" (operator hidden) vs "Status is active" (operator
        shown)
      </div>
    </div>
  );
