...
/** Locale object compatible with date-fns Locale type. */
export type DateLocale = {
  ...
  formatLong?: unknown;
  match?: unknown;
  options?: { weekStartsOn?: 0 | 1 | 2 | 3 | 4 | 5 | 6; firstWeekContainsDate?: 1 | 4 };
};

export interface DateFormatConfig {
  /** Format string for displaying dates in the UI (e.g., 'MMM d, yyyy') */
  displayFormat?: string;
  /** Format string for the internal value (e.g., 'yyyy-MM-dd') */
  valueFormat?: string;
  /** date-fns locale for formatting and parsing */
  locale?: DateLocale;
  /**
   * Custom parse function for non-standard formats like Unix timestamps.
   *
   * **Note**: When provided, `valueFormat` is ignored for parsing.
   * If not provided, uses date-fns parse with `valueFormat`.
   */
  parse?: (value: string) => Date | null;
  /**
   * Custom format function for non-standard formats like Unix timestamps.
   *
   * **Note**: When provided, `valueFormat` is ignored for formatting.
   * If not provided, uses date-fns format with `valueFormat`.
   */
