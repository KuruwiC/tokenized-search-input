...
describe('filterItems', () => {
  ...
  describe('maxResults option', () => {
    it('limits results to maxResults', () => {
      const result = filterItems(items, '', getTargets, { maxResults: 2 });
      expect(result.length).toBe(2);
    });

    it('returns all items when maxResults is larger than items', () => {
      const result = filterItems(items, '', getTargets, { maxResults: 100 });
      expect(result.length).toBe(items.length);
    });

    it('limits filtered results to maxResults', () => {
      const result = filterItems(items, 'a', getTargets, { maxResults: 1 });
      expect(result.length).toBe(1);
    });
  });
  ...
  describe('combined options', () => {
    it('applies matcher, minScore, and maxResults together', () => {
      const result = filterItems(items, 'act', getTargets, {
        matcher: fuzzy,
        minScore: 1,
        maxResults: 1,
