...
describe('normalizeDateValue', () => {
  ...
  it('uses custom format config', () => {
    const config = {
      valueFormat: 'dd/MM/yyyy',
    };
    expect(normalizeDateValue('5/3/2024', config)).toBe('05/03/2024');
  });
});
...
describe('normalizeDateTimeValue', () => {
  ...
  it('normalizes date-only input to full datetime', () => {
    // Date-only is parseable and gets normalized to full datetime with midnight time
