...
function createMockElement(
  id: string,
  options: { entryFocusable?: boolean; isAvailable?: boolean } = {}
): FocusableElement & { element: HTMLDivElement } {
  const { entryFocusable = true, isAvailable = true } = options;
  const focusFn = vi.fn();
...
describe('Focus Registry', () => {
  ...
  describe('element availability', () => {
    it('filters out unavailable elements', () => {
      const { result } = renderHook(() => useFocusRegistry());
      const available = createMockElement('available', { isAvailable: true });
      const unavailable = createMockElement('unavailable', { isAvailable: false });

      appendToContainer(container, available);
      appendToContainer(container, unavailable);
      result.current.register(available);
      result.current.register(unavailable);

      expect(result.current.getElements()).toHaveLength(1);
      expect(result.current.getElements()[0]?.id).toBe('available');
    });
  });
