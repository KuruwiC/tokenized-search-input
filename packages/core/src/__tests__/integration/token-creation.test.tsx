...
describe('Token Creation - User Journeys', () => {
  ...
  describe('FreeText token spacing', () => {
    ...
    it('creates consecutive freetext tokens with single space between them in serialized output', async () => {
      const onChange = vi.fn();
      ...
      // Insert first token
      editor.commands.insertFreeTextToken({ value: 'hello', quoted: false });
      await waitFor(() => {
        expect(screen.getByRole('group', { name: /Free text: hello/i })).toBeInTheDocument();
      });

      // Insert second token
      editor.commands.insertFreeTextToken({ value: 'world', quoted: false });
      await waitFor(() => {
        expect(screen.getByRole('group', { name: /Free text: world/i })).toBeInTheDocument();
      });

      // Get final serialized value
      const lastCall = onChange.mock.calls[onChange.mock.calls.length - 1];
      const snapshot = lastCall[0];

      // Should be "hello world" (single space between tokens)
      expect(snapshot.text).toBe('hello world');
    });

    it('creates three consecutive freetext tokens with correct spacing', async () => {
      const onChange = vi.fn();
      const editorRef = { current: null as TokenizedSearchInputRef | null };
      render(
        <TokenizedSearchInput
      ...
      // Insert tokens
      editor.commands.insertFreeTextToken({ value: 'one', quoted: false });
      await waitFor(() => {
        expect(screen.getByRole('group', { name: /Free text: one/i })).toBeInTheDocument();
      });

      editor.commands.insertFreeTextToken({ value: 'two', quoted: false });
      await waitFor(() => {
        expect(screen.getByRole('group', { name: /Free text: two/i })).toBeInTheDocument();
      });

      editor.commands.insertFreeTextToken({ value: 'three', quoted: false });
      await waitFor(() => {
        expect(screen.getByRole('group', { name: /Free text: three/i })).toBeInTheDocument();
      });
      ...
      expect(snapshot.text).toBe('one two three');
    });
  });
