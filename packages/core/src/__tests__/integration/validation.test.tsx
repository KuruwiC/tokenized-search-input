...
describe('Validation System Integration', () => {
  ...
  describe('Multiple rules with different actions', () => {
    it('applies different actions per rule', async () => {
      ...
      await waitFor(() => {
        const tokens = document.querySelectorAll('.node-filterToken');
        expect(tokens.length).toBe(3); // status:active, priority:high, priority:low
      });

      await waitFor(() => {
        const invalidTokens = document.querySelectorAll('.node-filterToken [data-invalid="true"]');
        expect(invalidTokens.length).toBe(1); // priority:low marked
      });
    });
  });
  ...
  describe('Custom validation rules', () => {
    it('supports custom validation logic', async () => {
      ...
      await waitFor(() => {
        const invalidTokens = document.querySelectorAll('.node-filterToken [data-invalid="true"]');
        expect(invalidTokens.length).toBe(1); // Third status is invalid
      });
    });
  });
  ...
  describe('setValue with validation', () => {
    ...
    it('deletes duplicates on setValue', async () => {
      ...
      await waitFor(() => {
        const tokens = document.querySelectorAll('.node-filterToken');
        expect(tokens.length).toBe(2); // status + priority (duplicate status deleted)
      });
    });
  });
  ...
  describe('pattern preset', () => {
    ...
    it('validates string value with pattern', async () => {
      // Test that pattern validates string value
      ...
      // Valid lowercase tag should pass
      const invalidTokens = document.querySelectorAll('.node-filterToken [data-invalid="true"]');
      expect(invalidTokens.length).toBe(0);
    });
  });
  ...
  describe('Edge cases', () => {
    ...
    it('handles rule that throws exception gracefully', async () => {
      ...
      // Editor should still render and tokens should not be marked invalid
      await waitFor(() => {
        const tokens = document.querySelectorAll('.node-filterToken');
        expect(tokens.length).toBe(1);
      });
    ...
    it('clears invalid state when remaining token is no longer duplicate after delete-existing', async () => {
      ...
      // After delete-existing, only the last token remains and should be valid (not marked invalid)
      await waitFor(() => {
        const tokens = document.querySelectorAll('.node-filterToken');
        expect(tokens.length).toBe(1);
      });

      // The remaining token should NOT be marked invalid
      const invalidTokens = document.querySelectorAll('.node-filterToken [data-invalid="true"]');
      expect(invalidTokens.length).toBe(0);
    });
    ...
    it('handles adjacent token deletions correctly', async () => {
      ...
      // Note: With two rules (Unique.rule('exact') and Unique.rule('key')), tokens are grouped by signature:
      // - Unique.rule('exact') creates group for 'status:is:active' (pos 1 and 4)
      // - Unique.rule('key') creates group for remaining tokens with same key
      // Each group keeps its first token, so 2 tokens remain.
      // To keep only 1, use a single Unique.rule('key') rule.
      render(
        <TokenizedSearchInput
          fields={testFields}
          ...
          validation={{
            rules: [
              // Using only Unique.rule('key') to ensure all status tokens are in one group
              Unique.rule('key', Unique.reject),
            ],
          }}
        />
      );
    ...
    it('unique with default strategy marks duplicates', async () => {
      // Unique.rule() defaults to Unique.mark strategy
      render(
        <TokenizedSearchInput
          fields={testFields}
          defaultValue="status:is:active status:is:inactive"
          validation={{
            rules: [Unique.rule('key')], // No strategy specified, uses Unique.mark
          }}
        />
      );
  ...
  describe('Unique.replace protects edited token', () => {
    it('unique rule returns violations with targets for duplicates', async () => {
      ...
      await waitFor(() => {
        ...
        expect(capturedViolations[0].targetCount).toBe(2); // 2nd and 3rd tokens are targets
      });
    });
  });
