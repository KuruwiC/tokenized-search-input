    ...
    <Token
      ...
      onBlur={() => {
        try {
          ...
          const latestValue = latestNode.attrs.value ?? '';
          const hasValue = !!latestValue.trim();

          if (!hasValue) return;

          const current: TokenEditableAttrs = {
            value: latestNode.attrs.value ?? '',
            operator: latestNode.attrs.operator ?? '',
            key: latestNode.attrs.key ?? '',
            confirmed: latestNode.attrs.confirmed ?? false,
            immutable: latestNode.attrs.immutable ?? false,
            invalid: latestNode.attrs.invalid ?? false,
            displayValue: latestNode.attrs.displayValue ?? null,
            startContent: latestNode.attrs.startContent ?? null,
            endContent: latestNode.attrs.endContent ?? null,
          };
