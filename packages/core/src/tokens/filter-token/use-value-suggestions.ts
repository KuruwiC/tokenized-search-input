...
import { parseDate } from '../../pickers/date-format';
import {
  closeSuggestion,
  getSuggestionState,
  openDateSuggestion,
...
export function useValueSuggestions({
  ...
  const handleValueInputFocus = () => {
    ...
    if (isEnumField && fieldDef?.type === 'enum' && fieldDef.enumValues) {
      ...
      const currentDate = value ? parseDate(value, dateFieldDef.formatConfig?.valueFormat) : null;
      openDateSuggestion(tr, fieldKey, currentDate, anchorPos);
      tr.setMeta('addToHistory', false);
      editor.view.dispatch(tr);
    } else if (isDateTimeField) {
      // DateTime field: show datetime picker
      const datetimeFieldDef = fieldDef as DateTimeFieldDefinition;
      const currentDate = value
        ? parseDate(value, datetimeFieldDef.formatConfig?.valueFormat)
        : null;
