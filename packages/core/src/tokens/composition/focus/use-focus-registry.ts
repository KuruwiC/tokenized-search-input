...
function getAvailableElements(elements: FocusableElement[]): FocusableElement[] {
  return elements.filter((el) => {
    if (!el.ref.current) return false;
    return el.isAvailable ? el.isAvailable() : true;
  });
}
...
export function useFocusRegistry(options: UseFocusRegistryOptions = {}): FocusRegistry {
  ...
  if (!registryRef.current) {
    const getElements = (): FocusableElement[] => {
      ...
      return sortByDomOrder(getAvailableElements(elements));
    };
