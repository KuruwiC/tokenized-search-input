...
import { isUTCValue } from '../pickers/date-format';
...
export interface SuggestionOverlayProps {
  ...
  onDateChange?: (date: Date | null, fieldKey: string, isUTC?: boolean) => void;
  onDateClose?: () => void;
...
export const SuggestionOverlay: React.FC<SuggestionOverlayProps> = ({
  ...
  const flushAndCancelPendingDateChange = useCallback(() => {
    ...
    const fieldKeyToUse = currentSuggestionState.fieldKey;
    const suggestionType = currentSuggestionState.type;
    ...
    if (fieldKeyToUse) {
      ...
      onDateChange?.(dateToUpdate, fieldKeyToUse, useUTC);
    }
  }, [editor, onDateChange]);
  ...
  useEffect(() => {
    ...
    if (!isDateOrDateTime) {
      setTokenInputValue('');
      // Reset UTC state when leaving datetime mode
      setIsUTC(false);
      return;
    }
    ...
    if (anchorPos !== null && anchorPos !== undefined) {
      ...
      // No anchor position but datetime type - reset UTC
      setIsUTC(false);
    }
  ...
  }, [editor, suggestionState?.anchorPos, suggestionState?.type]);
  ...
  const handleUTCChangeInternal = useCallback(
    (newIsUTC: boolean) => {
      ...
      if (currentDate && fieldKeyToUse) {
        onDateChange?.(currentDate, fieldKeyToUse, newIsUTC);
      }
    },
            ...
            {isPrepend && customItems.length > 0 && (
              <>
                <CustomSuggestionList
                  ...
                  paginationLabels={paginationLabels}
                />
                {fieldItems.length > 0 && <div className="tsi-divider" />}
              </>
            )}
            {fieldItems.length > 0 && (
              <FieldSuggestionList
