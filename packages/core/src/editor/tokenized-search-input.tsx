...
import { EMPTY_SNAPSHOT } from '../utils/query-snapshot';
import { diffSnapshots, notifyTokenObservers } from '../utils/token-events';
...
export interface TokenizedSearchInputProps {
   ...
   * Called after token event handlers (onTokenCreate/Delete/Update).
   */
  ...
  /**
   * Called when a new filter token is created.
   * Called before onChange.
   */
  onTokenCreate?: (token: QuerySnapshotFilterToken) => void;
  /**
   * Called when a filter token is deleted.
   * Called before onChange.
   */
  onTokenDelete?: (tokenId: string) => void;
  /**
   * Called when a filter token's value or operator is updated.
   * Called before onChange.
   */
...
export const TokenizedSearchInput = forwardRef<TokenizedSearchInputRef, TokenizedSearchInputProps>(
  function TokenizedSearchInput(
    ...
    // Track previous snapshot for token event diffing
    // Initialize with EMPTY_SNAPSHOT to fire create events for initial tokens
    ...
    const editor = useEditor({
      ...
      onUpdate: ({ editor: ed }) => {
        ...
        // Notify token event observers (before onChange)
        const events = diffSnapshots(prevSnapshotRef.current, snapshot);
        notifyTokenObservers(events, { onTokenCreate, onTokenDelete, onTokenUpdate });
