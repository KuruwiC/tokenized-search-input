...
import { createTokenFocusPlugin, getTokenFocusState } from '../plugins/token-focus-plugin';
import {
...
import { EMPTY_SNAPSHOT, getFilterTokens } from '../utils/query-snapshot';
import { areFilterTokenListsEqualExcludingFocused } from '../utils/token-events';
...
export const TokenizedSearchInput = forwardRef<TokenizedSearchInputRef, TokenizedSearchInputProps>(
  function TokenizedSearchInput(
    ...
    const confirmedTokensRef = useRef<QuerySnapshotFilterToken[]>([]);
    ...
    const editor = useEditor({
      ...
      onUpdate: ({ editor: ed }) => {
        ...
        if (onTokensChange) {
          ...
          const currentTokens = getFilterTokens(snapshot);
          ...
          const isEqual = areFilterTokenListsEqualExcludingFocused(
